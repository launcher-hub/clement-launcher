...
// Bind the add offline account button (crack / hors ligne).
const _settingsAddOfflineAccount = document.getElementById('settingsAddOfflineAccount')
if (_settingsAddOfflineAccount) {
    _settingsAddOfflineAccount.onclick = (e) => {
...
// Back-compat: if the template still has the Mojang button, bind it too.
const _settingsAddMojangAccount = document.getElementById('settingsAddMojangAccount')
if (_settingsAddMojangAccount) {
    _settingsAddMojangAccount.onclick = (e) => {
        switchView(getCurrentView(), VIEWS.login, 500, 500, () => {
            loginViewOnCancel = VIEWS.settings
            loginViewOnSuccess = VIEWS.settings
            loginCancelEnabled(true)
...
function populateAuthAccounts(){
    ...
    const selectedUUID = selected ? selected.uuid : null

    // Resolve containers lazily so missing sections do not crash.
    const msftEl = document.getElementById('settingsCurrentMicrosoftAccounts')
    const offlineEl = document.getElementById('settingsCurrentOfflineAccounts')
    const mojangEl = document.getElementById('settingsCurrentMojangAccounts')
    ...
    authKeys.forEach((val) => {
            ...
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    ...
                    <div class="settingsAuthAccountDetailPane">
                        ...
                        <div class="settingsAuthAccountDetailValue" style="font-family:monospace">${acc.uuid}</div>
                    </div>
                </div>
        ...
        if(acc.type === 'microsoft') {
            ...
            // Non-microsoft accounts are treated as offline when crack is enabled.
            // Otherwise, keep legacy behavior (mojang section) if present.
            offlineAuthAccountStr += accHtml
        }
    })

    if(msftEl){
        msftEl.innerHTML = microsoftAuthAccountStr
    }
    if(offlineEl){
        offlineEl.innerHTML = offlineAuthAccountStr
    }
    if(mojangEl){
        mojangEl.innerHTML = mojangAuthAccountStr
    }
}
...
function populateSettingsUpdateInformation(data){
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/3db00b4a-e148-4232-960d-37213473bd9b',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({runId:'pre-fix',hypothesisId:'H2',location:'settings.js:populateSettingsUpdateInformation',message:'Update tab refs status',data:{titleInitialized:typeof settingsUpdateTitle !== 'undefined',actionButtonInitialized:typeof settingsUpdateActionButton !== 'undefined'},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    if(data != null){
...
async function prepareSettings(first = false) {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/3db00b4a-e148-4232-960d-37213473bd9b',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({runId:'pre-fix',hypothesisId:'H2',location:'settings.js:prepareSettings',message:'prepareSettings called',data:{first,hasUpdateTitle:settingsUpdateTitle != null,hasUpdateActionButton:settingsUpdateActionButton != null},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    if(first){
        setupSettingsTabs()
